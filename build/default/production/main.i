...
# 24 "./SSD.h"
  ...
  extern unsigned char Num_Digit[2];
  extern unsigned char Select_Seg;
...
# 1 "./HEATER_AND_COOLER.h" 1
# 23 "./HEATER_AND_COOLER.h"
void Heating_and_Cooling_INIT(void);
void Disable_Heating_and_Cooling(void);
void Heating_and_Cooling(float * current_Temp , unsigned char * Temperature);
# 36 "main.c" 2
...
void main(void) {
    ...
    EX_Interrupt_INIT(ISR_CallBack);
    ADC_INIT();
    Master_INIT(100000);
    Timer_INIT();
    ...
    SSD_INIT();
    Buttons_INIT();
    Heating_and_Cooling_INIT();
    EEPROM_Read( 2 , 0xF0 , &Temperature );


    while(1)
    {
        Heating_and_Cooling( &current_Temp , &Temperature );
...
void decrease_Temp(void)
{
    if(Setting_Mode==1)
        ...
        EEPROM_Write( 2 , 0xF0 , Temperature );
        INT1=Sec1=Sec5=0;
        }
        else
        {
...
void ISR_CallBack(void)
{
    ...
    if(Sleep_Mode==0)
    {
        ...
        static unsigned int Heating_LED_Flag=0;
        static unsigned char Milli_Sec=0;


        if(INT1==1) {decrease_Temp();}


        if(INT2==1) {increase_Temp();}
        ...
        if(TMR1IF==1)
        {
            ...
            if(Milli_Sec==20)
            {
                Temp_average[i++]=Sense_Temp();
                Milli_Sec=0;
            }

            if(Heating_LED_Flag==200 )
            {
                if(RC5==1) {RB7=!RB7; }
                Heating_LED_Flag=0;
            }
